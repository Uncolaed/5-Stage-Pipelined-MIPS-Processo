...
-- Entity: completeCPU
-- Description:
-- Top-level integration for the pipelined MIPS processor.
-- Connects fetch, decode, execute, memory, and write-back paths.
--------------------------------------------------------------------------------
...
entity completeCPU is
  port(
    ...
    rstPC  : in std_logic;
    rstPL  : in std_logic;
    -- Debug outputs to observe pipeline results
    pcOut              : out std_logic_vector(31 downto 0);  -- Output of Fetch stage
    instructionOut     : out std_logic_vector(31 downto 0);  -- Output of Fetch stage
    aluResultOut       : out std_logic_vector(31 downto 0);  -- Output of Execute stage
    memReadDataOut     : out std_logic_vector(31 downto 0);  -- Output of Memory stage
    regWriteDataOut    : out std_logic_vector(31 downto 0);   -- Output of Write-back stage
     -- Decode stage outputs
    decodeMemToRegOut  : out std_logic;                      -- Decode stage: MemToReg signal
    decodeRegWriteOut  : out std_logic;                      -- Decode stage: RegWrite signal
    decodeJumpOut      : out std_logic;                      -- Decode stage: Jump signal
    decodeBranchOut    : out std_logic;                      -- Decode stage: Branch signal
    decodeMemReadOut   : out std_logic;                      -- Decode stage: MemRead signal
    decodeMemWriteOut  : out std_logic;                      -- Decode stage: MemWrite signal
    decodeRegDstOut    : out std_logic;                      -- Decode stage: RegDst signal
    decodeAluSrcOut    : out std_logic;                      -- Decode stage: AluSrc signal
    decodeAluOpOut     : out std_logic_vector(2 downto 0);   -- Decode stage: AluOp signal
    decodeJumpAddrOut  : out std_logic_vector(31 downto 0);  -- Decode stage: Jump address
    decodeReadData1Out : out std_logic_vector(31 downto 0);  -- Decode stage: Read data 1
    decodeReadData2Out : out std_logic_vector(31 downto 0);  -- Decode stage: Read data 2
    decodeExtdSignOut  : out std_logic_vector(31 downto 0);  -- Decode stage: Extended sign
    decodeRegRtOut     : out std_logic_vector(4 downto 0);   -- Decode stage: Register RT
    decodeRegRdOut     : out std_logic_vector(4 downto 0)    -- Decode stage: Register RD
    );
end completeCPU;
...
architecture behavioral of completeCPU is
  ...
  component Decode_instruction is
    port(
      ...
      aluOpOut2           : out std_logic_vector(2 downto 0);
      -- Instruction fields and decoded data
  ...
  component Execute_Stage is
    port(
      ...
      aluOpIn3          : in std_logic_vector(2 downto 0);
      -- Instruction fields and decoded data
  ...
  component MemoryOperations_Stage is
    port(
      clk              : in std_logic;
  ...
  signal sBranchAddrToFetch, sJumpAddrToFetch : std_logic_vector(31 downto 0);
  ...
  signal sAluOpToElab : std_logic_vector(2 downto 0);
    --Various Data
  ...
  signal sBranchToMemOp, sMemReadToMemOp, sMemWriteToMemOp, sZeroFlagToMemOp : std_logic;
    --Various Data
  signal sJumpAddrToMemOp, sBranchAddrToMemOp : std_logic_vector(31 downto 0);
  ...
  begin
    ...
    a_fetching : Fetch_Instruction
      port map (clk => clock, resetProgCounter => rstPC, resetPipeline1 => rstPL, muxBranchControlIn1 => sBranchToFetch, muxBranchExtIn1 => sBranchAddrToFetch,
    ...
    b_instrPartition : Decode_instruction
    ...
    c_elaboration : Execute_Stage
      port map (clk => clock, resetPipeline3 => rstPL, memToRegIn3 => sMemToRegToElab, regWriteIn3 => sRegWriteToElab,
        ...
        memReadOut3 => sMemReadToMemOp, memWriteOut3 => sMemWriteToMemOp, jumpAddrOut3 => sJumpAddrToMemOp, branchAddrOut3 => sBranchAddrToMemOp,
    ...
    d_memOp : MemoryOperations_Stage
      ...
      branchAddrIn4 => sBranchAddrToMemOp, zeroFlagIn4  => sZeroFlagToMemOp, aluResultIn4 => sAluResultToMemOp, readData2In4 => sReadData2ToMemOp,
      registerIn4 => sWriteRegToMemOp, memToRegOut4 => sMemToRegToMux, regWriteOut4 => sRegWriteFlagToFetch, jumpOut4 => sJumpToFetch,
      branchOut4 => sBranchToFetch, jumpAddrOut4 => sJumpAddrToFetch, branchAddrOut4 => sBranchAddrToFetch, aluResultOut4 => sAluResultToMux,
...
-- Connect internal signals to outputs
  pcOut <= sPcToPartition;                -- Output of Fetch stage
  instructionOut <= sInstructionToPartition;  -- Output of Fetch stage
  aluResultOut <= sAluResultToMemOp;      -- Output of Execute stage
  memReadDataOut <= sReadDataMemToMux;    -- Output of Memory stage
  regWriteDataOut <= sDataWriteToFetch;   -- Output of Write-back stage
  -- Decode stage outputs
  decodeMemToRegOut  <= sMemToRegToElab;  -- Decode stage: MemToReg signal
  decodeRegWriteOut  <= sRegWriteToElab;  -- Decode stage: RegWrite signal
  decodeJumpOut      <= sJumpToElab;      -- Decode stage: Jump signal
  decodeBranchOut    <= sBranchToElab;    -- Decode stage: Branch signal
  decodeMemReadOut   <= sMemReadToElab;   -- Decode stage: MemRead signal
  decodeMemWriteOut  <= sMemWriteToElab;  -- Decode stage: MemWrite signal
  decodeRegDstOut    <= sRegDstToElab;    -- Decode stage: RegDst signal
  decodeAluSrcOut    <= sAluSrcToElab;    -- Decode stage: AluSrc signal
  decodeAluOpOut     <= sAluOpToElab;     -- Decode stage: AluOp signal
  decodeJumpAddrOut  <= sJumpAddrToElab;  -- Decode stage: Jump address
  decodeReadData1Out <= sReadData1ToElab; -- Decode stage: Read data 1
  decodeReadData2Out <= sReadData2ToElab; -- Decode stage: Read data 2
  decodeExtdSignOut  <= sExtdSignToElab;  -- Decode stage: Extended sign
  decodeRegRtOut     <= sRegRtToElab;     -- Decode stage: Register RT
  decodeRegRdOut     <= sRegRdToElab;     -- Decode stage: Register RD
end behavioral;
